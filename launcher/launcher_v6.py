...
class AppLauncherGUI:
    def __init__(self, root):
        ...
        tk.Button(button_frame, text="一键关闭 A/B", command=self.terminate_processes, width=15).pack(side="left", padx=10)
    ...
    def start_a_file(self):
        ...
        try:
            # 启动 A 文件（指定 cwd，避免弹窗失败）
            process = subprocess.Popen(a_file_path, cwd=os.path.dirname(a_file_path))
            pid = process.pid
            self.log(f"A 文件已启动，PID: {pid}")

            # 启动 B 文件（使用 PowerShell 正确执行 .ps1 脚本）
            if os.path.exists(b_file_path):
                self.log(f"启动 B 文件：{b_file_path}")
                subprocess.Popen(
                    ["powershell", "-ExecutionPolicy", "Bypass", "-File", b_file_path],
                    cwd=os.path.dirname(b_file_path)
                )
            else:
